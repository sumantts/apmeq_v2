<?php
    ...
    if($ext == 'jpg' || $ext == 'jpeg' || $ext == 'png'){
		...
		$sql = "SELECT * FROM product_master WHERE prod_id = '" .$prod_id. "' ";
		$result = $mysqli->query($sql);

		if ($result->num_rows > 0) { 
			$row = $result->fetch_array(); 
			$all_images_en = $row['all_images'];	
            if($all_images_en != ''){
        ...
        //$upd_sql = "UPDATE product_master SET all_images = '" .$all_images_en. "' WHERE prod_id = '" .$prod_id. "' ";
